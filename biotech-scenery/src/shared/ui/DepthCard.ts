...
export class DepthCard extends Node {
  ...
  public constructor(
    ...
    const fill =
      options.fill ??
      (variant === 'dark' ? BiotechColors.panelDark : BiotechColors.panelFill)
    const stroke = variant === 'dark' ? BiotechColors.panelDarkStroke : BiotechColors.panelStroke
    const titleFill = variant === 'dark' ? '#ecf0f1' : BiotechColors.ink
