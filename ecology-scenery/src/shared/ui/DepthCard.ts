...
export class DepthCard extends Node {
  ...
  public constructor(
    ...
    const fill =
      options.fill ??
      (variant === 'dark' ? EcologyColors.panelDark : EcologyColors.panelFill)
    const stroke = variant === 'dark' ? EcologyColors.panelDarkStroke : EcologyColors.panelStroke
    const titleFill = variant === 'dark' ? '#ecf0f1' : EcologyColors.ink
