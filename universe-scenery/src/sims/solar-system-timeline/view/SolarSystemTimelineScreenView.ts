...
function makePillLabel(text: string, x: number, y: number, fill = 'rgba(248,250,252,0.92)', center = true): Node {
  ...
  const bg = new Rectangle(-5, -2, t.width + 10, t.height + 4, { cornerRadius: 4, fill, stroke: 'rgba(15,23,42,0.12)', lineWidth: 1 })
...
export class SolarSystemTimelineScreenView extends ScreenView {
  ...
  public constructor(model: SolarSystemTimelineModel, providedOptions?: Options) {
    ...
    this.guide = new GuidanceBanner(lb.width - m * 2, { title: SolarSystemTimelineStrings.guideTitleStringProperty.value, body: SCENARIO_GUIDE.explore })
    ...
    this.leftLearnTip = createPanelTip(SolarSystemTimelineStrings.learnMoreStringProperty.value, { width: leftW - 24, fontSize: 11, fill: UniverseColors.panelMuted })
    this.leftLearnTip.left = 12; this.leftLearnTip.top = this.teachingTriad.bottom + 22; leftCard.content.addChild(this.leftLearnTip)

    this.addChild(new StageBackdrop(this.stageLeft, this.stageTop, this.stageW, this.stageH, { top: '#070b18', bottom: '#102038' }))
    this.titleText = new Text(SolarSystemTimelineStrings.stageTitleStringProperty.value, { font: new PhetFont({ size: 18, weight: 'bold' }), fill: '#f8fafc', centerX: this.stageCenterX, top: this.stageTop + 10 }); this.addChild(this.titleText)
    this.captionText = new Text('', { font: new PhetFont({ size: 14, weight: 'bold' }), fill: '#86efac', centerX: this.stageCenterX, top: this.titleText.bottom + 4 }); this.addChild(this.captionText)
    ...
    this.tipCard.content.addChild(new Text(SolarSystemTimelineStrings.tipTitleStringProperty.value, { font: new PhetFont({ size: 12, weight: 'bold' }), fill: UniverseColors.accent, left: 14, top: 10 }))
    ...
    const scenarioHeader = controlSection(SolarSystemTimelineStrings.sectionScenarioStringProperty.value, contentW); panelContent.addChild(scenarioHeader)
    ...
    const timelineHeader = controlSection(SolarSystemTimelineStrings.sectionTimelineStringProperty.value, contentW); panelContent.addChild(timelineHeader)
    this.runningToggleBtn = new SoftButton(SolarSystemTimelineStrings.runningOnStringProperty.value, () => { model.togglePlay(); sounds.playPause(model.runningProperty.value) }, { width: contentW, height: btnH, fill: UniverseColors.accent, fontSize: 12, selected: true }); panelContent.addChild(this.runningToggleBtn)
    const timelineSlider = new DepthSlider(model.progressProperty, { min: 0, max: MAX_PROGRESS, width: contentW, label: SolarSystemTimelineStrings.timelineSliderStringProperty.value, format: (n) => `${Math.round(n) + 1}/${MAX_PROGRESS + 1}`, fill: '#34d399', onTick: () => { sounds.sliderTick(); model.scrubProgress(model.progressProperty.value) } }); panelContent.addChild(timelineSlider)
    const conditionsHint = controlHint(SolarSystemTimelineStrings.conditionsHintStringProperty.value, contentW); panelContent.addChild(conditionsHint)
    const displayHeader = controlSection(SolarSystemTimelineStrings.sectionDisplayStringProperty.value, contentW); panelContent.addChild(displayHeader)
    this.labelsBtn = new SoftButton(SolarSystemTimelineStrings.labelsOnStringProperty.value, () => { sounds.softClick(); model.showLabelsProperty.value = !model.showLabelsProperty.value }, { width: contentW, height: btnH, fill: '#64748b', fontSize: 11, selected: true }); panelContent.addChild(this.labelsBtn)
    const playbackHeader = controlSection(SolarSystemTimelineStrings.sectionPlaybackStringProperty.value, contentW); panelContent.addChild(playbackHeader)
    this.playPauseBtn = new SoftButton(SolarSystemTimelineStrings.playButtonStringProperty.value, () => { model.togglePlay(); sounds.playPause(model.runningProperty.value) }, { width: contentW, height: 38, fill: UniverseColors.accent, fontSize: 12 }); panelContent.addChild(this.playPauseBtn)
    const soundHeader = controlSection(SolarSystemTimelineStrings.sectionSoundStringProperty.value, contentW); panelContent.addChild(soundHeader)
    this.soundBtn = new SoftButton(SolarSystemTimelineStrings.soundOnStringProperty.value, () => { sounds.unlock(); const on = !model.soundEnabledProperty.value; model.soundEnabledProperty.value = on; sounds.toggle(on); if (on) sounds.button(); this.soundBtn.setLabel(on ? SolarSystemTimelineStrings.soundOnStringProperty.value : SolarSystemTimelineStrings.soundOffStringProperty.value) }, { width: contentW, height: btnH, fill: '#64748b', fontSize: 12 }); panelContent.addChild(this.soundBtn)
    const statusHeader = controlSection(SolarSystemTimelineStrings.sectionStatusStringProperty.value, contentW); panelContent.addChild(statusHeader)
    ...
    const learnTip = createPanelTip(SolarSystemTimelineStrings.learnMoreStringProperty.value, { width: contentW, fontSize: 11 }); panelContent.addChild(learnTip)
    ...
    model.scenarioProperty.link(() => {
      ...
      this.guide.setGuidance(SolarSystemTimelineStrings.guideTitleStringProperty.value, SCENARIO_GUIDE[model.scenarioProperty.value])
    ...
    model.runningProperty.link(() => {
      ...
      this.playPauseBtn.setLabel(running ? SolarSystemTimelineStrings.pauseButtonStringProperty.value : SolarSystemTimelineStrings.playButtonStringProperty.value)
      this.runningToggleBtn.setLabel(running ? SolarSystemTimelineStrings.runningOnStringProperty.value : SolarSystemTimelineStrings.runningOffStringProperty.value)
      this.runningToggleBtn.setSelected(running)
    })
    model.showLabelsProperty.link(() => { this.labelsBtn.setSelected(model.showLabelsProperty.value); this.labelsBtn.setLabel(model.showLabelsProperty.value ? SolarSystemTimelineStrings.labelsOnStringProperty.value : SolarSystemTimelineStrings.labelsOffStringProperty.value); this.labelsLayer.visible = model.showLabelsProperty.value })
    model.soundEnabledProperty.link((on) => sounds.setEnabled(on))
    model.starsProperty.link(() => { this.starsText.string = `${SolarSystemTimelineStrings.starsStringProperty.value} ${model.starsProperty.value}` })
  ...
  private drawStage(): void {
    ...
    const cx = ox + w * 0.5; const cy = oy + h * 0.48
    const ev = this.model.currentEvent
    const t = this.animT
    this.stageLayer.removeAllChildren(); this.labelsLayer.removeAllChildren()
    for (let i = 0; i < 34; i++) {
      this.stageLayer.addChild(new Circle(0.7 + (i % 3) * 0.35, { fill: 'rgba(255,255,255,0.45)', centerX: ox + 10 + (i * 51) % (w - 20), centerY: oy + 40 + (i * 37) % (h - 90) }))
    }
    if (ev.era === 'formation') {
      this.stageLayer.addChild(new Circle(70, { fill: 'rgba(255,152,0,0.25)', centerX: cx, centerY: cy }))
      this.stageLayer.addChild(new Circle(28, { fill: '#ffeb3b', centerX: cx, centerY: cy }))
      this.stageLayer.addChild(new Path(new Shape().ellipse(cx, cy, 110 + t * 6, 36 + t * 2, 0), { stroke: 'rgba(200,180,255,0.45)', lineWidth: 2 }))
      for (let i = 0; i < 6; i++) {
        const a = (i / 6) * Math.PI * 2 + t * 0.5
        const r = 70 + (i % 3) * 16
        this.stageLayer.addChild(new Circle(5 + (i % 2) * 2, { fill: '#78909c', centerX: cx + Math.cos(a) * r, centerY: cy + Math.sin(a) * r * 0.35 }))
      }
    } else if (ev.era === 'planets') {
      this.stageLayer.addChild(new Circle(18, { fill: '#ffeb3b', centerX: cx, centerY: cy }))
      const planets = [{ r: 40, size: 4, c: '#9e9e9e' }, { r: 58, size: 6, c: '#ff7043' }, { r: 78, size: 7, c: '#42a5f5' }, { r: 95, size: 5, c: '#ef5350' }, { r: 115, size: 10, c: '#ffb74d' }, { r: 132, size: 8, c: '#ffcc80' }]
      for (const p of planets) {
        this.stageLayer.addChild(new Path(new Shape().ellipse(cx, cy, p.r, p.r * 0.35, 0), { stroke: 'rgba(255,255,255,0.12)', lineWidth: 1 }))
        this.stageLayer.addChild(new Circle(p.size, { fill: p.c, centerX: cx + p.r, centerY: cy }))
      }
    } else if (ev.era === 'exploration') {
      this.stageLayer.addChild(new Circle(42, { fill: '#78909c', centerX: cx - 50, centerY: cy + 20 }))
      const rocketX = cx + 60; const rocketY = cy - 20 - Math.sin(t * 2) * 5
      this.stageLayer.addChild(new Path(new Shape().moveTo(rocketX, rocketY - 28).lineTo(rocketX + 11, rocketY + 18).lineTo(rocketX - 11, rocketY + 18).close(), { fill: '#eceff1' }))
      this.stageLayer.addChild(new Path(new Shape().moveTo(rocketX - 7, rocketY + 18).lineTo(rocketX, rocketY + 32).lineTo(rocketX + 7, rocketY + 18).close(), { fill: '#ff7043' }))
    } else {
      this.stageLayer.addChild(new Path(new Shape().moveTo(cx - 80, cy + 30).lineTo(cx + 40, cy - 40), { stroke: 'rgba(167,139,250,0.7)', lineWidth: 3 }))
      this.stageLayer.addChild(new Circle(10 + Math.sin(t * 3) * 2, { fill: '#a78bfa', centerX: cx + 50, centerY: cy - 45 }))
    ...
    if (this.model.showLabelsProperty.value) {
      this.labelsLayer.addChild(makePillLabel(ev.era.toUpperCase(), cx, oy + 48, eraColor(ev.era) + 'cc'))
      this.labelsLayer.addChild(makePillLabel(ev.yearLabel, ox + 14, oy + h - 28, 'rgba(248,250,252,0.92)', false))
    }
  ...
  private showQuiz(): void {
    this.miniQuiz.showQuiz(SolarSystemTimelineStrings.quizQuestionStringProperty.value, [
  ...
  public override step(dt: number): void {
    this.model.step(dt); this.particles.step(dt); this.animT += dt
