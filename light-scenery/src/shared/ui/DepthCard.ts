...
export class DepthCard extends Node {
  ...
  public constructor(
    ...
    const fill =
      options.fill ??
      (variant === 'dark' ? LightColors.panelDark : LightColors.panelFill)
    const stroke = variant === 'dark' ? LightColors.panelDarkStroke : LightColors.panelStroke
    const titleFill = variant === 'dark' ? '#ecf0f1' : LightColors.ink
