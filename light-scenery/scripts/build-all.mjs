...
const SIMS = ['laws-of-reflection', 'regular-vs-diffuse', 'plane-mirror-periscope', 'refraction-media', 'rainbow-dispersion', 'curved-mirrors']
mkdirSync(destDir, { recursive: true })
console.log('Typechecking…')
execSync('npx tsc --noEmit', { cwd: root, stdio: 'inherit' })
for (const sim of SIMS) {
  console.log(`\nBuilding ${sim}…`)
  execSync('npx vite build', { cwd: root, stdio: 'inherit', env: { ...process.env, LIGHT_SIM: sim } })
  const candidates = [
...
console.log('\nAll light SceneryStack sims built.')
