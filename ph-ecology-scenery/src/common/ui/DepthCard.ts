...
export class DepthCard extends Node {
  ...
  public constructor(
    ...
    const fill =
      options.fill ??
      (variant === 'dark' ? SimTheme.panelDark : SimTheme.panelFill)
    const stroke = variant === 'dark' ? SimTheme.panelDarkStroke : SimTheme.panelStroke
    const titleFill = variant === 'dark' ? '#ecf0f1' : SimTheme.ink
