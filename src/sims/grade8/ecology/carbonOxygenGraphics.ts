...
/** Shared agent artwork — also mirrored in ph-carbon-oxygen-scenery EcologyArt. */
export const AGENT_ART = {
  tree: svgUri(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  <defs>
  ...
  cow: svgUri(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  <defs>
  ...
  deer: svgUri(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  <defs>
  ...
  rabbit: svgUri(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  <defs>
  ...
  factory: svgUri(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  <defs>
  ...
  fungi: svgUri(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  <defs>
    <radialGradient id="cap" cx="40%" cy="30%" r="70%">
      <stop offset="0%" stop-color="#fca5a5"/>
      <stop offset="100%" stop-color="#dc2626"/>
    </radialGradient>
  </defs>
  <ellipse cx="32" cy="56" rx="10" ry="3" fill="#000" opacity="0.15"/>
  <path d="M14 36 Q32 8 50 36 Z" fill="url(#cap)"/>
  <ellipse cx="32" cy="36" rx="18" ry="5.5" fill="#fecaca"/>
  <rect x="28" y="36" width="8" height="18" rx="2.5" fill="#fef3c7"/>
  <circle cx="24" cy="26" r="2.4" fill="#fff" opacity="0.85"/>
  <circle cx="36" cy="22" r="3" fill="#fff" opacity="0.85"/>
  <circle cx="42" cy="30" r="1.8" fill="#fff" opacity="0.7"/>
</svg>`),
} as const
...
const art = {
  tree: loadImg(AGENT_ART.tree),
  cow: loadImg(AGENT_ART.cow),
  deer: loadImg(AGENT_ART.deer),
  rabbit: loadImg(AGENT_ART.rabbit),
  factory: loadImg(AGENT_ART.factory),
  fungi: loadImg(AGENT_ART.fungi),
}
...
export function drawAgent(
  ...
  if (kind === 'plant') {
    ...
    const img = variant === 0 ? art.cow : variant === 1 ? art.deer : art.rabbit
    const size = variant === 2 ? 48 : 54
    ctx.save()
    ctx.translate(x, y + bob)
    ctx.scale(breathe, breathe)
    if (img.complete) ctx.drawImage(img, -size / 2, -size + 4, size, size)
    ctx.restore()
  } else {
    const shake = rates.combustion > 0.2 ? Math.sin(time * 18) * 0.6 : 0
    if (rates.combustion > 0.15) {
      const g = ctx.createRadialGradient(x + 10, y - 50, 2, x + 10, y - 50, 32)
      g.addColorStop(0, 'rgba(148,163,184,0.4)')
      g.addColorStop(1, 'rgba(148,163,184,0)')
      ctx.fillStyle = g
      ctx.beginPath()
      ctx.arc(x + 10 + shake, y - 50, 32, 0, Math.PI * 2)
      ctx.fill()
    }
    ctx.save()
    ctx.translate(x + shake, y)
    if (art.factory.complete) ctx.drawImage(art.factory, -32, -60, 64, 64)
    // Animated smoke from chimney (right side of factory art)
    if (rates.combustion > 0.1) {
      for (let p = 0; p < 5; p++) {
        const t = (time * 0.65 + p * 0.28) % 1.5
        const px = 14 + Math.sin(time * 1.2 + p) * 5 + t * 12
        const py = -54 - t * 32
        const pr = 5 + t * 9
        ctx.beginPath()
        ctx.arc(px, py, pr, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(71,85,105,${(0.5 - t * 0.3) * Math.min(1, rates.combustion / 4)})`
        ctx.fill()
      }
    }
    ctx.restore()
