...
import { HeredityColors } from '../HeredityColors.js'
...
export class DepthCard extends Node {
  ...
  public constructor(
    ...
    const fill =
      options.fill ??
      (variant === 'dark' ? HeredityColors.panelDark : HeredityColors.panelFill)
    const stroke = variant === 'dark' ? HeredityColors.panelDarkStroke : HeredityColors.panelStroke
    const titleFill = variant === 'dark' ? '#ecf0f1' : HeredityColors.ink
