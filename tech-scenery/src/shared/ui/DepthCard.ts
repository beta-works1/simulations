...
import { TechColors } from '../TechColors.js'
...
export class DepthCard extends Node {
  ...
  public constructor(
    ...
    const fill =
      options.fill ??
      (variant === 'dark' ? TechColors.panelDark : TechColors.panelFill)
    const stroke = variant === 'dark' ? TechColors.panelDarkStroke : TechColors.panelStroke
    const titleFill = variant === 'dark' ? '#ecf0f1' : TechColors.ink
